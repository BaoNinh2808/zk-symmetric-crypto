...
func (c *MyCircuit) Define(api frontend.API) error {
	...
	// check consistency between KeyType and ValType; Key.Ref_index and Val.Ref_index
	for i := 0; i < split; i += 2 {
		...
		//Key.Ref_index = Val.Ref_index
		isEqualRefIndex := uapi.IsEqualU8(c.Data[i].Ref_index, c.Data[i+1].Ref_index)
		api.AssertIsEqual(isEqualRefIndex, 1)
	}
	...
	// check correct Length of Object Value in array part (if not object_val, then len = 0)
	for i := split; i < n; i++ {
		val_index := c.Data[i].Self_index
		total_Refs := frontend.Variable(0)
		for j := 3; j < split; j += 2 { //check the value in the key-val part
	...
	// // check criterias
	js.ROOT_KEYCHECK_Type(&c.Crits[0])
	js.KEYCHECK_Type(&c.Crits[1])
	js.ANONYMOUS_ARRAYPART_VALCHECK_Type(&c.Crits[2])
	js.KEYCHECK_Type(&c.Crits[3])
	js.ANONYMOUS_ARRAYPART_VALCHECK_Type(&c.Crits[4])
	js.KEYCHECK_Require(&c.Crits[5])
	js.KEYCHECK_Require(&c.Crits[6])
	js.KEYCHECK_Type(&c.Crits[7])
	js.KEYCHECK_Type(&c.Crits[8])
