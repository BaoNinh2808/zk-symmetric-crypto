...
type MyCircuit struct {
	...
	Criterias_Keys [1]BlockData `gnark:",public"`
...
func (c *MyCircuit) Define(api frontend.API) error {
	if c.Criterias_Keys[0] == nil {
		c.Criterias_Keys[0] = *NewBlock()
	}
	// c.Criterias_Keys[0] = *NewBlock()
	...
	for i := 0; i < 1; i++ {
		...
		// check val less
		isLess := bapi.IsLess(&c.Criterias_Vals[i], &val)
	...
	//check require
	//js.Require(&c.Criterias_Keys[0], c.Corresponding_Data_Index[0])
	//js.MinimumU8(&c.Criterias_Keys[0], &c.Criterias_Vals[0], c.Corresponding_Data_Index[0])
	js.Type(uints.NewU8(0), []uint8{NULL_KEY_TYPE, BOOL_KEY_TYPE, STRING_KEY_TYPE, NUMBER_KEY_TYPE, ARRAY_KEY_TYPE, OBJECT_KEY_TYPE})
	// js.MaxLength(uints.NewU8(1), uints.NewU8(1))
	js.MaxProperties(uints.NewU8(1), uints.NewU8(0))
	js.UniqueItems(uints.NewU8(1))
